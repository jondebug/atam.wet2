...
struct _EvDocumentModel
{
	...
	gint creator_mode : 1; //ADDED maybe change the 1
...
ev_document_model_class_init (EvDocumentModelClass *klass)
{
	...
	g_object_class_install_property (g_object_class,
					 ...
					 g_param_spec_boolean ("continuous",
							       ...
							       G_PARAM_READWRITE |
                                                               G_PARAM_STATIC_STRINGS));
	g_object_class_install_property (g_object_class, //ADDED
					 PROP_CREATOR_MODE,
				  ...
				  gboolean         mode)
{
	...
	if (mode == model->creator_mode)
		return;

	model->creator_mode = mode;
...
gboolean
ev_document_model_get_creator_mode (EvDocumentModel *model)//ADDED
{
	g_return_val_if_fail (EV_IS_DOCUMENT_MODEL (model), TRUE);
