...
setup_document_from_metadata (EvWindow *window)
{
	 ...
	 * show the first page. */
	ev_metadata_get_boolean (priv->metadata, "creator-mode", &creator_mode);//added
	page = ev_document_model_get_page (priv->model);
	n_pages = ev_document_get_n_pages (priv->document);
	if ((page == n_pages - 1)&&(!creator_mode))
...
static const GActionEntry actions[] = {
	...
	{ "creator-mode", NULL, NULL, "true", ev_window_cmd_creator_mode },
